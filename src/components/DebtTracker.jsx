...
export default function DebtTracker() {
  ...
  const settleDebt = async (person, amount, direction) => {
    ...
    const tx = pendingTransactions.find(t => t.person === person);

    if (!tx) {
      showErrorAlert('Error', 'No matching debt found');
      setIsSettling(false);
      return;
    }
    ...
    await db.transactions.add({
      ...
      accountId,
      timestamp: Date.now(),
      title: `Settlement - ${person}`
    });

    await db.transactions.update(tx.id, { status: 'paid' });
            ...
            {debts.oweMe.map(d => (
              <div key={d.person} className="flex justify-between items-center">
                ...
                <div className="text-right">
                  ...
                  <button
                    onClick={() => settleDebt(d.person, d.amount, 'owe_me')}
                    className="text-xs mt-1 text-emerald-600 font-medium"
                  >
                    Settle
                  </button>
                </div>
            ...
            {debts.iOwe.map(d => (
              <div key={d.person} className="flex justify-between items-center">
                ...
                <div className="text-right">
                  ...
                  <button
                    onClick={() => settleDebt(d.person, d.amount, 'i_owe')}
                    className="text-xs mt-1 text-rose-600 font-medium"
                  >
                    Pay
                  </button>
                </div>
