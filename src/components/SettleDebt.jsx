...
export default function SettleDebt() {
  ...
  const newBalance = selectedAccount
    ? (isReceiving ? selectedAccount.balance + amount : selectedAccount.balance - amount)
    : 0;
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
          <div className={`w-20 h-20 mx-auto rounded-full flex items-center justify-center mb-4 ${isReceiving ? 'bg-emerald-100' : 'bg-rose-100'
            ...
            {isReceiving ? (
              ...
              <FiAlertCircle size={40} className="text-rose-600" />
            )}
          </div>
          <h2 className="text-2xl font-bold text-gray-800">{person}</h2>
          <p className="text-gray-500 mt-1">
            {isReceiving ? 'owes you money' : 'you owe money to'}
          </p>
        </div>
          ...
          {selectedAccount && (
            <div className="bg-gray-50 rounded-xl p-4 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className={`font-semibold ${newBalance >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
              ...
              {isInsufficient && (
                <div className="bg-amber-50 rounded-lg p-2 mt-2">
                  <p className="text-xs text-amber-700 text-center">
                    ⚠️ Your account balance will become negative after this transaction
                  </p>
                </div>
              )}
            </div>
